...
export default function Main(vido: Vido, props = {}) {
  ...
  onDestroy(
    state.subscribeAll(
      [
        ...
        '$data.innerHeight', // for calculate scroll area
  ...
  function partialReload() {
    if (debug) console.log('Partial reload fired.', {}); // eslint-disable-line no-console
    generateTree();
    prepareExpanded();
    minimalReload();
  }
  onDestroy(
    state.subscribeAll(
      ['config.list.rows.*.parentId', 'config.chart.items.*.rowId', 'config.list.rows.*.expanded'],
      partialReload,
      { bulk: true }
    )
  );
  ...
  function updateVisibleItems(time: DataChartTime = state.get('$data.chart.time'), multi = state.multi()) {
    ...
    for (const item of visibleItems) {
      ...
      multi = multi.update(
        `$data.chart.items.${item.id}`,
        function ($data: ItemData) {
          if (debug) console.log('Update visible item', { $data, item }); // eslint-disable-line no-console
          if (!$data) return;
